...
Vector* VectorCreate(size_t elementSize, size_t initialSize) {
	...
	vec->elements = malloc(sizeof(vec->elementSize) * initialSize);
...
void VectorAdd(Vector* vector, void* element) {
	if (vector->size >= vector->totalSize) {
		...
		void* tmpElements = realloc(vector->elements, vector->elementSize + (vector->size + 50));

		if (tmpElements == NULL) {
			printf("Attempt to increase vector size with reallocation failed!!!\n");
			return;
		}
	...
	memcpy(vector->elements + (vector->elementSize * vector->size), element, sizeof(vector->elementSize));
	vector->size++;
}

void* VectorRemove(Vector* vector, int index) {
	if (index >= vector->size) {
		printf("Attempted to index %d which is out of range of vector with size %d", index, vector->size);
		return;
	}
